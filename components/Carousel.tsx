    ...
    <section className="flex w-full flex-col gap-4 overflow-hidden" ref={emblaRef}>
      <div className="flex ">
        {slides.slice(0,5).map((item) => (
            <figure key={item._id} className='carousel_box' onClick={() => router.push(`/podcasts/${item.podcast[0]?.pocastId}`)}>
                ...
                <div className='glassmorphism-black relative z-10 flex flex-col rounded-b-xl p-4'>
                    ...
                    <p className='text-white-2 font-normal text-12'>{item.name}</p>
                </div>
            </figure>
        ))}
      </div>
